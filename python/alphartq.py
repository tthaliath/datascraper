...
json_data = json.load(open('/home/bitnami/tthaliath/scripts/github/tickerlick/tradevantage/tradevantage/conf/config.json'))
...
try:
        con  = mdb.connect(user='dbmasteruser', password='neha2005',host='ls-3ae674dec7649e30442d77d1201959cbcf14bd77.cc152ykrnoem.us-west-2.rds.amazonaws.com',database='tickmaster')
	#con = mdb.connect(dbhost,tickmasterdb,dbuser,dbpassword)
	cur = con.cursor()
	for ticker in (tickerlist.split(",")):
		...
		if mo is None:
			...
			if mo is None:	
				...
				rtq = mo.group(1)
				rtq = re.sub(',', '', rtq)
				cur.execute("insert into secpricert (ticker,price_date,rtq,low_price_14,high_price_14) VALUES (%s,%s,%s,%s,%s)", (ticker,price_date,rtq,mo.group(2),mo.group(3)))	
				con.commit()
		else:
