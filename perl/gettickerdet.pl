...
sub getpricehistory
{
 my $ticker_id = shift;
 my $no_of_days = shift;
 my $resulthtml  = '<table border="1" cellpadding="1" cellspacing="1" width="70%" align="center"><tr><td><table>';
$dbh = DBI->connect('dbi:mysql:tickmaster',$DBUSER,$DBPASSWORD) or die "Connection Error: $DBI::errstr\n";
 $sql ="select a.price_date, a.close_price, a.dma_10, a.dma_50, a.dma_200,ema_12,ema_26,ema_diff,ema_macd_9 from tickerprice a where a.ticker_id = $ticker_id ORDER BY a.price_date DESC LIMIT 0,$no_of_days;";
 $resulthtml .= "<tr><td>price_date</td><td>close_price</td><td>dma_10</td><td>dma_50</td><td>dma_200</tr>";
 $sth = $dbh->prepare($sql);
 $sth->execute or die "SQL Error: $DBI::errstr\n";
 while (@row = $sth->fetchrow_array) {
 $resulthtml .= "<tr><td>$row[0]</td><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</tr><td>$row[5]</tr><td>$row[6]</tr><td>$row[7]</tr><td>$row[8]</tr>";
} 
