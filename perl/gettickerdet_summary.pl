 ...
 sub getmain1
{
  ...
  #print "$content\n";
   ...
   #close (C); 
   #exit;
 # <div class="title"><h2>iShares MSCI Emerging Markets (EEM)</h2> 
 #   if ($content =~ m/Market Cap:<\/th><td class\=\"yfnc_tabledata1\">N\/A<\/td>/) {return "NOMARKETCAP";}
if ($content =~ m/.*?<div class="title"><h2>(.*?)\(.*?h2>*?time_rtq_ticker.*?><span.*?>(.*?)<\/span>.*?Prev Close.*?<td class.*?>(.*?)<\/td>.*?1y Target Est.*?<td class.*?>(.*?)<\/td>.*?52wk Range.*?<span.*?>(.*?)<\/span> - <span>(.*?)<\/span>.*?Volume.*?<span.*?>(.*?)<\/span>.*?Market Cap\:.*?<span.*?>(.*?)<\/span>.*?P\/E .*?\(ttm\).*?<td class.*?>(.*?)<\/td>.*?EPS .*?\(ttm\).*?<td class.*?>(.*?)<\/td>.*?Div.*?Yield.*?<td class.*?>(.*?)<\/td>/)
 {
...
#{
      ...
      $tickmain =  $1."\t".$2."\t".$3."\t".$4."-".$5."\t".$6."\t".$7."\t".$8."\t".$9."\t".$10."\t".$11;
      $tickmain =~ s/<.*?>//g;
      }
      return   $tickmain;
}
...
sub getmain2
{
  ...
  my ($tickmain);
  #if ($content =~ m/Market Cap:<\/th><td class\=\"yfnc_tabledata1\">N\/A<\/td>/) {return "NOMARKETCAP";}
if ($content =~ m/.*?<div class="title"><h2>(.*?)\(.*?h2>.*?time_rtq_ticker.*?><span.*?>(.*?)<\/span>.*?Prev Close.*?<td class.*?>(.*?)<\/td>.*?1y Target Est.*?<td class.*?>(.*?)<\/td>.*?52wk Range\:<\/th><td class\=\"yfnc_tabledata1\">(.*?)<\/td>.*?Volume.*?<span.*?>(.*?)<\/span>.*?Market Cap\:.*?<span.*?>(.*?)<\/span>.*?P\/E .*?\(ttm\).*?<td class.*?>(.*?)<\/td>.*?EPS .*?\(ttm\).*?<td class.*?>(.*?)<\/td>.*?Div.*?Yield.*?<td class.*?>(.*?)<\/td>/)
 {
...
#{
      ...
      $tickmain =  $1."\t".$2."\t".$3."\t".$4."-".$4."\t".$5."\t".$6."\t".$7."\t".$8."\t".$9."\t".$10;
      #$tickmain =~ s/<.*?>//g;;
...
sub transformtickerdet
{
 ...
 #my ($Ticker,$name,$sector,$industry,$LastTrade,$PrevClose,$yTargetEst,$wkRange,$vol,$MarketCap,$PE,$EPS)= split("\t",$str);
 my ($Ticker,$name,$LastTrade,$PrevClose,$yTargetEst,$wkRange,$vol,$MarketCap,$PE,$EPS)= split("\t",$str);
