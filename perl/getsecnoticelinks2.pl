...
open (F,"</home/tthaliath/tickerlick/daily/sec/seclist.csv");
#$sql = "select ticker_id,ticker from tickermaster where price_flag = 'Y' and tflag = 'N'";
#print "$sql\n";
#my $sth = $dbh->prepare($sql);
#$sth->execute();
while (<F>) {	
        chomp;
	($ticker_id,$ticker) = split (/\,/,$_);
        #print "$ticker\n";
