...
#my %tickhash = getResults($ticker);
my ($EBITDA,$dbh,$diff,$diffhigh,$difflow,$perdiff,$perdiffhigh,$perdifflow,$rowhtml2,$sql,$sth,%tickerhash,@row);
sub getResults
{
my ($ticker) = uc shift;
my (%tickhash) ;
...
if ($content)
{
#print "1:$ticker: summary page downlaod successful\n";
#print "$content\n";
$content =~ s/\n//g;
...
$tickprofile = getetfprofile($ticker);
...
# }
  ...
  if ($tickmain =~ /^(.*?)\tNOMARKETCAP/)
  {
     $tickhash{Ticker} = $ticker;
     $tickerhash{LastTrade} = $1;
     $tickhash{Marketcap} = 0;
  }
  else
  {
   %tickhash =  transformtickerdet($tickdata);
   $tickhash{Marketcap} = 1;
  }
 return %tickhash;
}
}
 ...
 sub getmain1
{
  ...
  #print "$content\n";
   ...
   #exit;
    ...
    if ($content =~ m/Market Cap:<\/th><td class\=\"yfnc_tabledata1\">N\/A<\/td>/) {
     #return last price if exists
     if ($content =~ m/.*?time_rtq_ticker.*?><span.*?>(.*?)<\/span>/)
     {
        $tickmain = $1."\tNOMARKETCAP";
     }
     else
     {
       $tickmain = "NOMARKETCAP";
     }
     return $tickmain;
   }
...
sub getprofile
{
 ...
 $sql ="select comp_name from tickermaster where ticker = '$ticker';";
...
($Ticker,$LastTrade,$PrevClose,$yTargetEst,$wkRange,$vol,$MarketCap,$PE,$EPS)= split("\t",$str);
if ($Ticker)
{
...
#brokerage_name      | rating_date | updown | rating_from    | rating_to | price_from | price_to 
 my $resulthtml  = '<table border="1" cellpadding="1" cellspacing="1" width="70%" align="center" style="border-collapse: collapse;">';
 $resulthtml  .= '<tr bgcolor="#00FFFF"><th colspan=12>Analyst Opinion/Estimates</th></tr>';
 $resulthtml .= '<tr bgcolor="#00FFFF"><td>Date</td><td>Brokerage Name</td><td>Rating</td><td>Rating From</td><td>Rating To</td><td>Price From</td><td>Price To</td></tr>';
$dbh = DBI->connect('dbi:mysql:tickmaster',$DBUSER,$DBPASSWORD) or die "Connection Error: $DBI::errstr\n";
 $sql ="select  rating_date,brokerage_name,updown,rating_from,rating_to,price_from, price_to from rating_master where ticker_id = $ticker_id and rating_date > '$start_date' order by rating_date desc limit 10;";
 $sth = $dbh->prepare($sql);
 $sth->execute or die "SQL Error: $DBI::errstr\n";
 while (@row = $sth->fetchrow_array)
 {
