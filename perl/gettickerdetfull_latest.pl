...
$sql = "select a.price_date, a.close_price, (ema_diff - ema_macd_9) as macdmid, (ema_diff_5_35 - ema_macd_5) as macd,b.rsi_14  as rsi, (((dma_20 + (2 * dma_20_sd))) - ((dma_20 -  (2 * dma_20_sd)))) as bollidiff, (a.close_price - (dma_20 - (2 * dma_20_sd))) as pricebottom, (a.close_price - (dma_20 + (2 * dma_20_sd))) as pricetop,sma_3_3_stc_osci_14,ema_diff, ema_macd_9,dma_20,dma_20_sd from tickerprice a, tickerpricersi b, tickermaster c  where a.ticker_id = $ticker_id and c.ticker_id = a.ticker_id and a.ticker_id = b.ticker_id and a.price_date = b.price_date ORDER BY a.price_date DESC LIMIT 0,$no_of_days;";
 ...
 #$resulthtml  .= '<tr bgcolor="#00FFFF"><th colspan=2>Price History</th><th colspan=6>MACD Signal and Crossover</th><th colspan=6>RSI</th><th colspan=3>Bollinger</th></tr>';
...
push (@arrdata, $rowhtml);
 ...
 #macd(12,26,9)signal
#print "<tr><td>$row[0]</td><td>$row[9]</td><td>$row[10]</td><td>$row[2]</td></tr>";
#$row[2] = sprintf("%.4f", $row[2]);
#$a = $row[9];
#$b  = $row[10];
if ($row[9] > $row[10])
{
...
#$row[9] = sprintf("%.4f", $row[9]);
#$row[10] = sprintf("%.4f", $row[10]);
#$macmid = $row[9] - $row[10];
$macmid = sprintf("%.4f", $macmid);
#$row[2] = $macmid;
...
$dma_20 = sprintf("%.3f", (4*$row[12]/$row[11]));
if  ( $dma_20 <= 0.1)
{
  $signal5= 'Neutral';
...
{
   $resulthtml .=  $arrdata[$recno].$arrdata1[$recno].$arrcrossover1[$recno].$arrdata2[$recno].$arrcrossover2[$recno].$arrdata3[$recno].$arrdata7[$recno].$arrdata5[$recno].$arrdata6[$recno].$arrdata9[$recno];
