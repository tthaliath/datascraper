...
use Data::Dumper;
my ($dbh,$sth2,$sth1);
my $dbh = DBI->connect('dbi:mysql:tickmaster',$DBUSER,$DBPASSWORD) or die "Connection Error: $DBI::errstr\n";
my (%tick) = getResults($ARGV[0]);
sub getResults
{
my ($ticker) = uc shift;
my (%tickhash) ;
my $url = 'https://query.yahooapis.com/v1/public/yql?q=select%20LastTradePriceOnly,Open,DaysLow,DaysHigh,Volume,Symbol%20from%20yahoo.finance.quotes%20where%20symbol%20in%20(%22'.$ticker.'%20and%20startDate="2017-05-12"%20and%20endDate="2017-05-25")&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&callback=';
