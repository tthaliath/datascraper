...
#use DB_File ;
#our (%etfhash);
#tie %etfhash, "DB_File", "/home/tickerlick/cgi-bin/etfsecs" or die "Cannot open file 'etfsecs': $!\n";
my (%rephash);
...
else
{
 ...
 $content = get("http://ca.finance.yahoo.com/q/pr?s=$ticker+Profile");
 ...
 sub getmain1
{
  ...
  my ($mktcapflag) = 1; 
  my ($mktcaptext) = "N/A";
    if ($content =~ m/Market Cap:<\/th><td class\=\"yfnc_tabledata1\">N\/A<\/td>/) {$mktcapflag = 0;}
if (!$mktcapflag)
{
...
sub getsecfilings
{
 ...
 my (%rephashflag);
 $dbh = DBI->connect('dbi:mysql:tickmaster',$DBUSER,$DBPASSWORD) or die "Connection Error: $DBI::errstr\n";
 $sql ="select file_url, file_type, file_date from sec_filing where ticker_id = $ticker_id order by rating_date desc";
 my $resulthtml  = '<table border="1" cellpadding="1" cellspacing="1" width="70%" align="center" style="border-collapse: collapse;">';
 $resulthtml  .= '<tr bgcolor="#00FFFF"><th colspan=12>Latest SEC Filings</th></tr>';
 $resulthtml .= '<tr bgcolor="#00FFFF"><td>Type</td><td>File Date</td></tr>';
 $sth = $dbh->prepare($sql);
 $sth->execute or die "SQL Error: $DBI::errstr\n";
 while (@row = $sth->fetchrow_array)
 {
      $resulthtml .= "<tr><td><a href=".$row[0].">".$row[1]."</a></td><td>" . $row[2] ."</td></tr>";
 }
 $sth->finish;
 $resulthtml .= "</table>";
 return $resulthtml;
}
#get latest sec filings for all
sub getsecfilingsall
{
 my (%rephashflag);
 $dbh = DBI->connect('dbi:mysql:tickmaster',$DBUSER,$DBPASSWORD) or die "Connection Error: $DBI::errstr\n";
 $sql ="select file_url, file_type, file_date from sec_filing order by ticker, rating_date desc";
 my $resulthtml  = '<table border="1" cellpadding="1" cellspacing="1" width="70%" align="center" style="border-collapse: collapse;">';
 $resulthtml  .= '<tr bgcolor="#00FFFF"><th colspan=12>Latest SEC Filings</th></tr>';
 $resulthtml .= '<tr bgcolor="#00FFFF"><td>Type</td><td>File Date</td></tr>';
 $sth = $dbh->prepare($sql);
