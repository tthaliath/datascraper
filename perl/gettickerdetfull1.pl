...
use DBI;
my (%rephash);
$rephash{'OS'} = 'MACD (5,35,5) Oversold';
$rephash{'OB'} ='MACD (5,35,5) Overbought';
$rephash{'RS'} ='RSI Oversold';
$rephash{'RB'} = 'RSI Overbought';
$rephash{'SS'} ='Stochastic Oversold';
$rephash{'SB'} = 'Stochastic Overbought';
$rephash{'BBU'} = 'Bollinger Oversold';
$rephash{'BBE'} = 'Bollinger Overbought';
...
sub getpricehistory
{
 my $ticker_id = shift;
 my $no_of_days = shift;
 my ($signal,$crossoverlast,$rowhtml,$signal1,@rowhtml1,$signal2,@rowhtml2);
 my ($signallast) = 0;
 my ($signallast1) = 0;
 my ($signallast2) = 0;
 my ($rowprev) = 0;
 ...
 my ($signalhtml6) = "";
 my ($signalhtml7) = ""; 
 my (@arrdata,@arrcrossover,@arrdata2,@arrcrossover2,@arrdata3,@arrdata4,@arrdata5,@arrdata6,@arrdata7);
 my (%crossoverhash) = ( 
                         Bullish => Buy,
                         Bearish  => Sell,
                         nochange => '&nbsp;',
...
$sql = "select a.price_date, a.close_price, (ema_diff - ema_macd_9) as macdmid, (ema_diff_5_35 - ema_macd_5) as macd,b.rsi_14  as rsi, (((dma_20 + (2 * dma_20_sd))) - ((dma_20 -  (2 * dma_20_sd)))) as bollidiff, (a.close_price - (dma_20 - (2 * dma_20_sd))) as pricebottom, (a.close_price - (dma_20 + (2 * dma_20_sd))) as pricetop,b.rsistoch_14 from tickerprice a, tickerpricersi b, tickermaster c  where a.ticker_id = $ticker_id and c.ticker_id = a.ticker_id and a.ticker_id = b.ticker_id and a.price_date = b.price_date ORDER BY a.price_date DESC LIMIT 0,$no_of_days;";
 ...
 #$resulthtml  .= '<tr bgcolor="#00FFFF"><th colspan=2>Price History</th><th colspan=6>MACD Signal and Crossover</th><th colspan=2>RSI</th><th colspan=3>Bollinger</th></tr>';
$resulthtml  .= '<tr bgcolor="#00FFFF"><th colspan=2>Price History</th><th colspan=3>MACD Long</th><th colspan=3>MACD Short</th><th colspan=2>RSI</th><th colspan=2>RSIStochastic</th><th colspan=3>Bollinger</th></tr>';
 $resulthtml .= '<tr bgcolor="#00FFFF"><td>Price Date</td><td>Close Price</td><td>MACD (12,26,9)</td><td>Trend</td><td>Crossover</td><td>MACD (5,35,5)</td><td>Trend</td><td>Crossover</td><td>RSI</td><td>Signal</td><td>RSIStochastic</td><td>Signal</td><td>Bollinger</td><td>Volatility</td><td>Signal</td></tr>';
 $sth = $dbh->prepare($sql);
 ...
 {
  $rowcount++;
 $rowhtml = "<tr><td>$row[0]</td><td>$row[1]</td>";
...
{
  $signal1 = 'Bullish';
   $signalhtml1 = '<font color="#05840C">Bullish</font>';
}
else
{
  $signal1 = 'Bearish';
 $signalhtml1 = '<font color="#FF0000">Bearish</font>';

}
$row[2] = sprintf("%.3f", $row[2]);

$rowhtml1 = "<td>$row[2]</td><td>$signalhtml1</td>";

push (@arrdata1, $rowhtml1);

if (!$signallast1)
{
 #print "4:$rowcount,$rowprev2,$signallast2,$signal2\n";
 $signallast1 = $signal1;
 $arrcrossover1[$rowcount] = "<td>$crossoverhash{'nochange'}</td>";

}
else
{
if ($signallast1 ne $signal1) #crossover
 {
     $rowprev1 = $rowcount - 2;
      #print "5:$rowcount,$rowprev2,$signallast2,$signal2\n";

     if ($crossoverhash{$signallast1} eq 'Buy')
     {
       $arrcrossover1[$rowprev1] = '<td><b><font color="#05840C">'.$crossoverhash{$signallast1}.'</font></b></td>';
     }
     elsif ($crossoverhash{$signallast1} eq 'Sell')
      {
        $arrcrossover1[$rowprev1] = '<td><b><font color="#FF0000">'.$crossoverhash{$signallast1}.'</font></b></td>';
       }
       else
       {
        $arrcrossover1[$rowprev1] = "<td>$crossoverhash{$signallast1}</td>";
       }
     $arrcrossover1[$rowcount - 1] = "<td>$crossoverhash{'nochange'}</td>";
     $signallast1 = $signal1;
 }
  else
  {
    ...
    {
        $arrcrossover1[0] = "<td>$crossoverhash{'nochange'}</td>";
    }
...
$rowhtml2 = "<td>$row[3]</td><td>$signalhtml2</td>";
...
{
  $signal3= 'Oversold';
   $signalhtml3 = '<font color="#05840C">Bullish</font>';
}
elsif ($row[4] > 70)
{
  $signal3 = 'Overbought';
 $signalhtml3 = '<font color="#FF0000">Bearish</font>';
...
$rowhtml3 = "<td>$row[4]</td><td>$signalhtml3</td>";
...
$rowhtml5 = "<td>$row[5]</td><td>$signalhtml5</td>";
...
{
  $signal7= 'Oversold';
   $signalhtml7 = '<font color="#05840C">Bullish</font>';
}
elsif ($row[8] > 70)
{
  $signal7 = 'Overbought';
 $signalhtml7 = '<font color="#FF0000">Bearish</font>';
...
$rowhtml7 = "<td>$row[8]</td><td>$signalhtml7</td>";
...
{
   $resulthtml .=  $arrdata[$recno].$arrdata1[$recno].$arrcrossover1[$recno].$arrdata2[$recno].$arrcrossover2[$recno].$arrdata3[$recno].$arrdata7[$recno].$arrdata5[$recno].$arrdata6[$recno];
...
$dbh = DBI->connect('dbi:mysql:tickmaster',$DBUSER,$DBPASSWORD) or die "Connection Error: $DBI::errstr\n";
 #$sql ="select a.price_date, a.close_price,ema_diff,ema_macd_9, (ema_diff - ema_macd_9) as signalstrength, ema_diff_5_35,ema_macd_5, (ema_diff_5_35 - ema_macd_5) as signalstrength2 from tickerprice a where a.ticker_id = $ticker_id ORDER BY a.price_date DESC LIMIT 0,$no_of_days;";
$sql = "select a.price_date, a.close_price, (ema_diff_5_35 - ema_macd_5) as macd,sma_3_3_stc_osci_14 as stoch,b.rsi_14  as rsi, dma_20_sd, (((dma_20 + (2 * dma_20_sd))) - ((dma_20 -  (2 * dma_20_sd)))) as bollidiff from tickerprice a, tickerpricersi b, tickermaster c  where c.ticker = '$ticker' and c.ticker_id = a.ticker_id and a.ticker_id = b.ticker_id and a.price_date = b.price_date ORDER BY a.price_date DESC LIMIT 0,$no_of_days;";
 $sth = $dbh->prepare($sql);
 $sth->execute or die "SQL Error: $DBI::errstr\n";
#macd(5,35,5)signal
 while (@row = $sth->fetchrow_array)
...
elsif ($row[2] <  0 && $signallast eq "Buy") #crossover
 {
  $signalhash{'macd535'}= "$signaldate\t$signallast";
  last;
 }
 elsif ($row[0] >  0 && $signallast eq "Sell") #crossover
 {
  $signalhash{'macd535'}= "$signaldate\t$signallast";
  last;
 }
...
sub getratinghistory
{
 ...
 while (@row = $sth->fetchrow_array)
 {
     $resulthtml1 .= "<tr><td>$row[0]</td><td>$row[1]</td><td>$ratinghash{$row[2]}</td><td>$row[3]</td><td>$row[4]</td><td>$row[5]</td><td>$row[6]</td></tr>";
     $flag = 1;
 }
 if ($flag)
 {
