...
my   $tickdesc = "";
my $tickprofile = "";
#my   $content = get("http://in.finance.yahoo.com/q?s=$ticker");
my   $content = get("https://www.google.com/finance/historical?output=csv&q=cldr");
print "$content\n";
exit 1;
 if ($content =~ /.*?<span class\=\"last\-value\">(.*?)<\/span.*?<span class\=\"text low\">(.*?)<\/span>.*?<span class\=\"text high\">(.*?)<\/span>.*?<span class\=\"text low\">(.*?)<\/span>.*?<span class\=\"text high\">(.*?)<\/span>/s)
...
if ($content =~ m/.*?Last Trade\:.*?<span.*?>(.*?)<\/span>.*?52wk Range.*?<td.*?>(.*?) - (.*?)<\/td>.*?Volume.*?<span.*?>(.*?)<\/span>/)
 {
