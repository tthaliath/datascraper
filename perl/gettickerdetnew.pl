...
if ($content)
{
if ($content =~ m/^(.*)?>Headlines.*?(More\sHeadlines.*)$/s)
{
  #print "headlines\n";
...
if ($content =~ m/^(.*)?More\sHeadlines.*?More Press Releases?(.*)$/s)
{
   #print "headlines2\n";
   $content = $1.$2;
  #print "11111111111111111111111:$1\n";
   #print "22222222222222222222222:$2\n";
}
$content =~ s/\n//g;
...
elsif ($content =~ m/Day\'s Range\:.*?<td class.*?><span>N\/A<\/span> \- <span>N\/A<\/span>/)
{
#print "no:2\n";
 $tickhash{invalidticker} = 1;
 return %tickhash;
...
if ($content =~ /\>NAV[\&|\:]|SPDR|Market Vectors|\sETN\s|Direxion|ProShare|EGShares|PowerShares|Global\sX|Sprott|PIMCO|Cambria|QuantShares|DBX\s|\sETF\s\($ticker\)|AlphaDEX|FactorShares|FlexShares|iShares\sMSCI/)
{
$asset_class = 'ETF';
if ($content =~ /\sETN\s/ && $content !~ /\>NAV[\&|\:]/)
{
 $asset_class = 'ETN';
#treat it as equity
if ($content =~ m/52wk Range\:<\/th><td class\=\"yfnc_tabledata1\">N\/A<\/td>/)
{
$tickmain = getmainetn2(\$content);
}
else
{
 $tickmain = getmainetn1(\$content);
 }
}
 elsif ($content =~ m/52wk Range.*?td class.*?>N\/A<\/td>/ )
{
if ($content =~ m/Beta\:/ ){$betaflag = 1;} 
...
$tickprofile = getetfprofile(\$content);
 ...
 %tickhash =  transformtickerdetetf($tickdata,$asset_class,$betaflag); 
 return %tickhash;
}
else
{
...
sub getSignal2
{
 ...
 my (%rephashflag);
 $dbh = DBI->connect('dbi:mysql:tickmaster',$DBUSER,$DBPASSWORD) or die "Connection Error: $DBI::errstr\n";
 $sql ="select report_flag from report where ticker_id = $ticker_id and report_flag in ('OS','OB','SS','SB','RS','RB')";
 $sth = $dbh->prepare($sql);
 $sth->execute or die "SQL Error: $DBI::errstr\n";
 while (@row = $sth->fetchrow_array)
 {
